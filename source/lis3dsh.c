...
void set_OutputDataRate(lis3dsh_register odr_selection)
{
	lis3dsh_register tmp;
	tmp=read_register(LIS3DSH_CTRL_REG4_ADDR);
	tmp&=0x0f;
	tmp|=odr_selection;
	write_register(LIS3DSH_CTRL_REG4_ADDR,tmp);
}

void AxesEnable(lis3dsh_register axes)
{
	lis3dsh_register tmp;
	tmp=read_register(LIS3DSH_CTRL_REG4_ADDR);
	tmp&=0xf8;
...
{
	lis3dsh_register tmp;
	tmp=read_register(LIS3DSH_CTRL_REG3_ADDR);
	tmp|=(1<<_LIS3DSH_DR_EN)|
		 (1<<_LIS3DSH_IEA)  |
		 (1<<_LIS3DSH_INT1_EN);
	write_register(LIS3DSH_CTRL_REG3_ADDR,tmp);
}

void ReadAxes(LIS3DSH_OutXYZTypeDef* axes)
{
	uint8_t tmp=0;
	while(tmp==0)
		{
			tmp=read_register(LIS3DSH_STATUS_ADDR);
			tmp&=(1<<_LIS3DSH_ZYXDA);
		}

	read_block((lis3dsh_register*)axes,LIS3DSH_OUT_X_L_ADDR,6);
}

lis3dsh_register read_register(lis3dsh_register addres)
{
	...
	LIS3DSH_CS_LOW();
	sendbyte(addres);
	buffer=sendbyte(DUMMY_BYTE);
	LIS3DSH_CS_HIGH();

	return buffer;
}

void write_register(lis3dsh_register addres,lis3dsh_register value)
{
	LIS3DSH_CS_LOW();
	sendbyte(addres);
	sendbyte(value);
...
void write_block(lis3dsh_register* buf,lis3dsh_register addres,lis3dsh_register size)
{
	while(size > 0x00)
	{
		write_register(addres,*buf);
		size--;
		buf++;
		addres++;
	}
...
void read_block(uint8_t* buf, uint8_t addres, uint16_t size)
{
	while(size > 0x00)
	  {
	    *buf = read_register(addres);
	    size--;
	    buf++;
	    addres++;
	  }
...
static uint8_t sendbyte(uint8_t byte)
{
  ...
  {
    if((LIS3DSHTimeout--) == 0) return timeout_usercallback();
  }
...
uint8_t timeout_usercallback(void)
{
